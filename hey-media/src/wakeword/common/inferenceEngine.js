class InferenceEngine {
  ...
  sequencePresent() {
    ...
    this.label_history = this.dropOldEntries(
      curr_time,
      this.label_history,
      this.inference_window_ms
    );
    ...
    for (let i = 0; i < this.label_history.length; i++) {
      ...
      if (label === target_label) {
        // move to next entry
        target_state += 1;
        if (target_state === this.inference_sequence.length) {
          // detected if the last index
          return true;
        }
      ...
      } else if (curr_label === label) {
        // continue with the previous entry
        last_valid_timestamp = curr_timestemp;
      } else if (
        last_valid_timestamp + this.tolerance_window_ms <
        curr_timestemp
      ) {
        curr_label = null;
        target_state = 0;
  ...
  accumulateArray(history_array) {
    let accum_history = [];
    for (let j = 0; j < this.num_class; j++) {
  ...
  getPrediction(curr_time) {
    this.pred_history = this.dropOldEntries(
      curr_time,
      this.pred_history,
      this.smoothing_window_ms
    );
