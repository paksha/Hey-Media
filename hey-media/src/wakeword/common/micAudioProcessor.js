...
class MicAudioProcessor {
  constructor(config) {
    ...
    if (
      ...
      navigator.getUserMedia = window.webkitGetUserMedia;
  ...
  getMicPermission() {
    ...
    var successCallback = function (micStream) {
      ...
      _micProc.micSource = _micProc.audioContext.createMediaStreamSource(
        micStream
      );
    ...
    var errorCallback = function (err) {
      console.log(
        "Initializing microphone has failed. Falling back to default audio file",
        err
      );
  ...
  initDownSampleNode() {
    ...
    this.downSampledBufferSize =
      (this.sampleRate / this.browserSampleRate) * this.srcBufferSize;
    ...
    this.downSampleNode.onaudioprocess = function (audioProcessingEvent) {
      ...
      var downSampledData = interpolateArray(
        inputData,
        _micProc.downSampledBufferSize
      );
      ...
      if (_micProc.data.length > _micProc.windowSize + _micProc.paddingSize) {
        _micProc.data.splice(
          0,
          _micProc.data.length - (_micProc.windowSize + _micProc.paddingSize)
        );
      }
    };
  }
...
export default MicAudioProcessor;
