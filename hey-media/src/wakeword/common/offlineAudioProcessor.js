...
class OfflineAudioProcessor {
  constructor(config, audioData) {
    ...
    this.audioContext = new OfflineAudioContext(
      1,
      samplePerWindow + config.micAudioProcessorConfig.paddingSize,
      this.sampleRate
    );
  ...
  initBufferSourceNode() {
    let audioSourceBuffer = this.audioContext.createBuffer(
      1,
      this.audioContext.length,
      this.sampleRate
    );
  ...
  getMFCC() {
    ...
    this.audioContext
      ...
      .then(function (renderedBuffer) {
        ...
        _offlineProc.extracted_features = _offlineProc.extracted_features.slice(
          0,
          _offlineProc.inputWidth
        );
        ...
        if (_offlineProc.extracted_features.length < _offlineProc.inputWidth) {
          while (
            _offlineProc.extracted_features.length !== _offlineProc.inputWidth
          ) {
            _offlineProc.extracted_features.push(
              new Array(_offlineProc.melBands).fill(0)
            );
          }
        }

        var extracted_features = util.transposeFlatten2d(
          _offlineProc.extracted_features
        );
...
export default OfflineAudioProcessor;
