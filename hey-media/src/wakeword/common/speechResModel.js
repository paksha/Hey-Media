...
class SpeechResModel {
  constructor(modelName, config) {
    ...
    for (let i = 2; i < this.config["n_layers"] + 1; i++) {
      if (i % 2 === 0) {
    ...
    for (let i = 0; i < this.config["n_layers"] + 1; i++) {
  ...
  loadWeight(layers, weights, pytorch = true) {
    ...
    function reformatConvKernel(weight) {
      ...
      for (
        var outFilterIndex = 0;
        outFilterIndex < weight.length;
        outFilterIndex++
      ) {
        for (
          let inFilterIndex = 0;
          ...
          for (
            let rowsIndex = 0;
            rowsIndex < weight[0][0].length;
            rowsIndex++
          ) {
            for (
              let colsIndex = 0;
              colsIndex < weight[0][0][0].length;
              colsIndex++
            ) {
              reformat[colsIndex][rowsIndex][inFilterIndex].push(
                weight[outFilterIndex][inFilterIndex][rowsIndex][colsIndex]
              );
            }
          }
        }
    ...
    for (var key in layers) {
      ...
      if (key.includes("bn")) {
        ...
        if (processedWeights[scaleWeightKey]) {
          w.push(
            tf.tensor1d(processedWeights[scaleWeightKey]["scale"], "float32")
          );
        } else {
